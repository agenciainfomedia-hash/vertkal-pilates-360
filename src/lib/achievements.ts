...
export const achievements: Achievement[] = [
  ...
  },
  {
    ...
    icon: Flame,
    category: 'progresso',
    condition: { type: 'phase_completed', value: 'Perda de Peso Sustentável' },
  ...
  },
  {
    ...
    icon: Dumbbell,
    category: 'progresso',
    condition: { type: 'phase_completed', value: 'Definição Muscular' },
  ...
  },
  {
    ...
    icon: Zap,
    category: 'vip',
    condition: { type: 'days_completed', value: 'vip_first' },
  ...
  },
  {
    ...
    icon: Medal,
    category: 'progresso',
    condition: { type: 'phase_completed', value: 'Consolidação e Integração' },
  ...
  },
  {
    ...
    icon: Award,
    category: 'vip',
    condition: { type: 'days_completed', value: 'all_vip' },
  ...
  return achievements.filter(achievement => {
    ...
    try {
      switch (achievement.condition.type) {
        case 'days_completed':
          return safeUserStats.completedDays.length >= (achievement.condition.value as number)
        case 'streak':
          return safeUserStats.currentStreak >= (achievement.condition.value as number)
        case 'phase_completed':
          return safeUserStats.completedPhases.includes(achievement.condition.value as string)
        case 'vip_unlock':
          return safeUserStats.isVIP
        case 'challenge_completed':
          return safeUserStats.completedChallenges.length >= (achievement.condition.value as number)
        default:
          return false
      }
    } catch (error) {
...
export const calculateTotalPoints = (unlockedAchievements: Achievement[]): number => {
...
// Função para obter próxima conquista
  ...
  try {
    ...
    return nextAchievements.sort((a, b) => {
      if (a.condition.type === 'days_completed' && b.condition.type === 'days_completed') {
        return (a.condition.value as number) - (b.condition.value as number)
      }
